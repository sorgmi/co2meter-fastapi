...
_DEFAULT_PORT = '1201'
_DEFAULT_INTERVAL = 20  # seconds
_DEFAULT_NAME = 'co2'
_INIT_TIME = 10  # time to initialize and calibrate device
...
#@app.get("/chart/")
#@app.get("/chart/{name>}")
@app.get("/chart/{name}/{freq}")
def chart_co2_temp(request: Request, name=None, freq='24H'):
...
#############################################################################
@app.route("/")
def dashboard_plotly(request: Request):
    # Get list of files
...
def start_server():
    """ Runs FastAPI instance using command line arguments """
    ...
    if not options.no_server:
        uvicorn.run(app, host="0.0.0.0", port=1201)
